<script lang="ts">
  import type { ConcentradorAreasParsed, EstudianteRowArea, NotasDetalladoPayload, Area } from './types'
  import type { ConcentradorPayload } from './api'
  import { theme } from './themeStore'
  import VirtualRows from './VirtualRows.svelte'

  export let parsed: ConcentradorAreasParsed | null
  ...
  export let currentOrden: string[]
  ...
  $: filteredEstudiantes = (() => {
    if (!parsed) return [] as EstudianteRowArea[]
    const list = parsed.estudiantes
    if (!search.trim()) return list
    const q = search.toLowerCase()
    return list.filter((est: EstudianteRowArea) => est.nombres.toLowerCase().includes(q))
  })()

  function valorPeriodo(est: EstudianteRowArea, itemAbrev: string, periodo: string) {
    const a = est.areas.find(a => a.area === itemAbrev)
    if (!a) return ''
  ...
  function getItemName(abreviatura: string) {
    if (!parsed || !parsed.areas) return abreviatura
    const item = parsed.areas.find(a => a.abreviatura === abreviatura)
